...
def get_world_points(depth, intrinsics, extrinsics):
    ...
    a = np.linalg.inv(intrinsics)
    b = np.vstack((x, y, np.ones_like(x))) * depth
...
class Dataset:
    def __init__(self, conf):
        ...
        if os.path.exists(os.path.join(self.data_dir, 'mask')):
        ...
        self.depth_lis = sorted(glob(os.path.join(self.data_dir, 'depth/*.png')))
        a = np.stack([cv.imread(im_name) for im_name in self.depth_lis])
        self.depth_np = np.stack(
            [cv.imread(im_name) for im_name in self.depth_lis]) / 256.0
        self.depth_np = self.depth_np[:, :, :, :1]
        self.depth_np = self.depth_np.squeeze()
        self.depth = torch.from_numpy(self.depth_np.astype(np.float32)).cpu()

        self.pts = []
        for i in range(self.n_images):
            ext = get_pose_inv(self.pose_all[i].detach().cpu().numpy())
            pts = get_world_points(self.depth_np[i], self.intrinsics_all[i], ext)
            pts_torch = torch.from_numpy((pts.astype(np.float32))).cpu()
            self.pts.append(pts_torch)
        self.pts = torch.stack(self.pts).reshape(self.n_images, self.H, self.W, -1)
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        pts_gt = self.pts[img_idx][(pixels_y, pixels_x)]  # batch_size, 3
        ...
        return torch.cat([rays_o.cpu(), rays_v.cpu(), color, mask[:, :1], pts_gt], dim=-1).cuda()
